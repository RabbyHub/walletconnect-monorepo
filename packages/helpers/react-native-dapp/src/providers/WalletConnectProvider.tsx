...
export default function WalletConnectProvider({
  ...
  const sessionStorageKey = React.useMemo(() => `${storageOptions.rootStorageKey}:session`, [
    storageOptions,
  ]);
  ...
  const connectToWalletService = React.useCallback(
    async (walletService: WalletService, uri?: string): Promise<void> => {
      ...
      const maybeRedirectUrl =
        typeof redirectUrl === "string" ? `&redirectUrl=${encodeURIComponent(redirectUrl)}` : "";
    ...
    async function shouldCreateConnector(
      ...
      nextConnector.on(ConnectorEvents.CALL_REQUEST_SENT, async (error: unknown) => {
        ...
        if (Platform.OS === "android") {
          ...
          const walletService: WalletService | undefined = await storage.getItem(
